...
Service_brook(){
	if [[ ${release} = "centos" ]]; then
		if ! wget --no-check-certificate "https://raw.githubusercontent.com/liu1xba23d/doubi-SSR/master/other/brook_centos" -O /etc/init.d/brook; then
		...
		if ! wget --no-check-certificate "https://raw.githubusercontent.com/liu1xba23d/doubi-SSR/master/other/brook_debian" -O /etc/init.d/brook; then
			echo -e "${Error} Brook服务 管理脚本下载失败 !" && exit 1
		fi
		chmod +x "/etc/init.d/brook"
...
Update_Shell(){
	...
	sh_new_ver=$(wget --no-check-certificate -qO- "https://raw.githubusercontent.com/liu1xba23d/doubi-SSR/master/brook.sh"|grep 'sh_ver="'|awk -F "=" '{print $NF}'|sed 's/\"//g'|head -1) && sh_new_type="softs"
	[[ -z ${sh_new_ver} ]] && sh_new_ver=$(wget --no-check-certificate -qO- "https://raw.githubusercontent.com/liu1xba23d/doubi-SSR/master/brook.sh"|grep 'sh_ver="'|awk -F "=" '{print $NF}'|sed 's/\"//g'|head -1) && sh_new_type="github"
	...
	if [[ ${sh_new_ver} != ${sh_ver} ]]; then
		...
		if [[ ${yn} == [Yy] ]]; then
			if [[ ${sh_new_type} == "softs" ]]; then
				wget -N --no-check-certificate https://raw.githubusercontent.com/liu1xba23d/doubi-SSR/master/brook.sh && chmod +x brook.sh
			else
				wget -N --no-check-certificate https://raw.githubusercontent.com/liu1xba23d/doubi-SSR/master/brook.sh && chmod +x brook.sh
			fi
